...
% boundary
%Notice
%This program has been modified to apply to p1-wave and big magnetic field
%and small magnetic field at the same time.
...
% We will often change the value of magnetic field strength
E = 8.352*10^-5/27.211385     ;  %a.u. 83.52 uev
F = 2.91*10^2/(5.142206*10^11);  %a.u. 291 v/m
B = 81*10^-6/(2.35*10^5)    ;  %a.u. 1.1 uT
...
% connect the phi_pattern and phi_sys
x_boun =[];
y_boun =[];
...
% calculate the data around the boundary
num_phi_fit =num_phi ; 
num_theta_fit =100;
...
for j = linspace(0,num_phi_fit-1,num_phi_fit)
    ...
    for i  = linspace(1,num_theta_fit,num_theta_fit)
        ...
        radius = ((xSol_num(tx_test,phi_fit_sys,initial_theta(i)) - xmn(1))^2 ...
...
for i = linspace(0,num_phi_fit-1,num_phi_fit)
    ...
    [fit_test_p1_real, radius_test11] = prepareCurveData(fit_test_p_real,radius_test);
    [fit_test_p1_imag, radius_test12] = prepareCurveData(fit_test_p_imag,radius_test);
   
    [fit_test_m1_real ,radius_test21] = prepareCurveData(fit_test_m_real,radius_test);
    [fit_test_m1_imag ,radius_test22] = prepareCurveData(fit_test_m_imag,radius_test); 
    ...
    [f_test_p_real ,gof_test_p_r] = fit(radius_test11,fit_test_p1_real,'poly2') ;
    [f_test_p_imag ,gof_test_p_i] = fit(radius_test12,fit_test_p1_imag,'poly2') ;
...
%     hold off;
    [f_test_m_real ,gof_test_m_r] = fit(radius_test21,fit_test_m1_real,'poly2');
    [f_test_m_imag ,gof_test_m_i] = fit(radius_test22,fit_test_m1_imag,'poly2');
...
%     radius_exp = linspace(radius_test(1),3*(radius_test(end) - radius_test(1))+radius_test(end),300);
    radius_exp = linspace(radius_test(1),1.4*10^-3,200);
